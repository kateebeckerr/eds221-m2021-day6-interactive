...
```{r}
# Read in World Bank development indicator data and metadata, have to read_ not read., read_ has more options, doesnt initialy showup when reading in its in a data sub folder not in main, starts in root o fproj by using here 
...
-   year should be once column and multiple variables trapped into series name col.

-   each variable in a single col, each observation in single row, one value in a single cell

-   data wrangling sequence up to 7 or 8 steps based on readability

### melt years first

```{r}
wb_indicators_long <- wb_indicators %>% 
  tidyr::pivot_longer(cols = '2001 [YR2001]':'2020 [YR2020]', # Which columns to squish, need quation if you refer to col that might be problematic startign with number or having space, lets r know its a col name 
...
wb_data_clean <- wb_indicators_long %>% 
  ...
  dplyr::select(-year_chr, -'Country Code', -'Series Code') # This drops a few redundant columns (caution here...best to leave things if you're not sure), dont need country code becasue fine with naame but might want it, can select ranges, sequences of variabels but could ask exlue variables, negative implies remove, select is columns and filter is filter or exluding rows base don whether it attaches your ocnditions or not, dont need quatiations around year_chr because its already created and doesnt have spaces 
...
Notice that we have multiple variables that were measured (our different indicators)(indicators...access.co2...etc) all in a single column. This is a scenario where there are multiple variables in a single column. To be **Tidy**, we want each variable to live in just *one* column.

That means we're going to need to widen this data. We'll do that using `tidyr::pivot_wider()`.

```{r}
wb_data_tidy <- wb_data_clean %>% 
  tidyr::drop_na('Series Name') %>% #nothing to do with data being tidy, jsut any value missing a series you msut drop 
  tidyr::pivot_wider(names_from = 'Series Name', values_from = indicator_value) %>% # Pivot to wide format
  dplyr::rename(county = "Country Name", 
                access_fuels_pp = "Access to clean fuels and technologies for cooking (% of pop)",
                acccess_electricity = "access to electricity",  
                co2_emissions_kt = "CO2",
                fossil_fuel_cons_pt = "fossil fuel consumption",
                water_stress = "level of water stress") %>%
  dplyr::mutate(year = as.numeric(year))
#independent of position, deosnt matter where they are, renaming col based on namess 


#if columns switched around its not dependant on location
#not changing this based on position 

view(wb_data_tidy)
```

```{r}
ggplot(data = wb_data_clean, aes(x = year, y = co2_emissions_kt)) + 
...
#redo steps that were given in key
